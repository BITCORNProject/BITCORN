...
TMPDIR_PREFIX = "bitgreen_func_test_"
...
class BitCornTestFramework(metaclass=BitCornTestMetaClass):
    ...
    def main(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave bitgreends and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop bitgreends after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bitgreen-cli instead of RPC for all commands")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: bitgreends were not stopped and may still be running")
    ...
    def add_nodes(self, num_nodes, extra_args=None, *, rpchost=None, binary=None):
        ...
        if binary is None:
            binary = [self.options.bitgreend] * num_nodes
        ...
        for i in range(num_nodes):
            self.nodes.append(TestNode(
                ...
                bitgreend=binary[i],
                bitgreen_cli=self.options.bitgreencli,
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bitgreend test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self, wait=0):
        """Stop multiple bitgreend test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bitgreend's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    rpchost=None,
                    timewait=self.rpc_timeout,
                    bitgreend=self.options.bitgreend,
                    bitgreen_cli=self.options.bitgreencli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i)  # Overwrite port/rpcport in bitgreen.conf
    ...
    def skip_if_no_bitgreend_zmq(self):
        """Skip the running test if bitgreend has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bitgreend has not been built with zmq enabled.")
    ...
    def skip_if_no_cli(self):
        """Skip the running test if bitgreen-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bitgreen-cli has not been compiled.")

    def is_cli_compiled(self):
        """Checks whether bitgreen-cli was compiled."""
