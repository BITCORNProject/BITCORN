 ...
 */
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool* fNewBlock, bool* fSpamNode) LOCKS_EXCLUDED(cs_main);
...
public:
    ...
    bool AcceptBlockHeader(
        ...
        CBlockIndex** ppindex,
        bool fProofOfStake) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
};
...
public:
    ...
    bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool* fAmountSpam, bool fCheckPoS = true) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
