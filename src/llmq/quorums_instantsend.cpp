        ...
        if (quorumSigningManager->GetVoteForId(llmqType, id, otherTxHash)) {
            if (otherTxHash != tx.GetHash()) {
                LogPrintf("CInstantSendManager::%s -- txid=%s: input %s is conflicting with islock %s\n", __func__,
                         tx.GetHash().ToString(), in.prevout.ToString(), otherTxHash.ToString());
                return false;
            }
            alreadyVotedCount++;
        }
    ...
    for (size_t i = 0; i < tx.vin.size(); i++) {
        ...
        if (quorumSigningManager->AsyncSignIfMember(llmqType, id, tx.GetHash())) {
            LogPrintf("CInstantSendManager::%s -- txid=%s: voted on input %s with id %s\n", __func__,
                      tx.GetHash().ToString(), in.prevout.ToString(), id.ToString());
        }
    }
...
{
    if (sporkManager.IsSporkActive(SPORK_7_INSTANTSEND_AUTOLOCKS))
        return false;
        ...
        if (!llmq::chainLocksHandler->HasChainLock(pindexMined->nHeight, pindexMined->GetBlockHash())) {
            if (printDebug) {
                LogPrint(BCLog::INSTANTSEND, "CInstantSendManager::%s -- txid=%s: outpoint %s too new and not ChainLocked. nTxAge=%d, nInstantSendConfirmationsRequired=%d\n", __func__,
                         txHash.ToString(), outpoint.ToString(), nTxAge, nInstantSendConfirmationsRequired);
            }
            return false;
        }
    }
        ...
        for (auto& in : tx->vin) {
            ...
            if (id == recoveredSig.id) {
                LogPrint(BCLog::INSTANTSEND, "CInstantSendManager::%s -- txid=%s: got recovered sig for input %s\n", __func__,
                         txid.ToString(), in.prevout.ToString());
                break;
            }
        }
    }
        ...
        for (auto& in : islock.inputs) {
            ...
            if (otherIsLock != nullptr) {
                LogPrintf("CInstantSendManager::%s -- txid=%s, islock=%s: conflicting input in islock. input=%s, other islock=%s, peer=%d\n", __func__,
                         islock.txid.ToString(), hash.ToString(), in.ToString(), ::SerializeHash(*otherIsLock).ToString(), from);
            }
        }
...
void CInstantSendManager::UpdateWalletTransaction(const uint256& txid, const CTransactionRef& tx)
{
    ...
    if (GetWallets().front()->UpdatedTransaction(txid)) {
        ...
        for (auto& in : islock.inputs) {
            ...
            if (it->second->GetHash() != islock.txid) {
                ...
                LogPrintf("CInstantSendManager::%s -- txid=%s, islock=%s: mempool TX %s with input %s conflicts with islock\n", __func__,
                         islock.txid.ToString(), hash.ToString(), it->second->GetHash().ToString(), in.ToString());
            }
        }
        ...
        for (auto& in : islock.inputs) {
            ...
            for (auto it = its.first; it != its.second; ++it) {
                ...
                LogPrintf("CInstantSendManager::%s -- txid=%s, islock=%s: mined TX %s with input %s and mined in block %s conflicts with islock\n", __func__,
                          islock.txid.ToString(), islockHash.ToString(), conflictTxid.ToString(), in.ToString(), info.pindexMined->GetBlockHash().ToString());
