...
UniValue gobject_submit(const JSONRPCRequest& request)
{
    ...
    if (govobj.GetObjectType() == GOVERNANCE_OBJECT_TRIGGER) {
        if (fMnFound) {
            ...
            LogPrint(BCLog::GOBJECT, "Object submission rejected because node is not a masternode\n");
        ...
        LogPrint(BCLog::GOBJECT, "Object submission rejected because fee tx not provided\n");
    ...
    bool fMissingConfirmations;
    {
        ...
        if (!govobj.IsValidLocally(strError, fMissingMasternode, fMissingConfirmations, true) && !fMissingConfirmations) {
            LogPrint(BCLog::GOBJECT, "Object submission rejected because object is not valid - hash = %s, strError = %s\n", strHash, strError);
    ...
    if (!governance.MasternodeRateCheck(govobj)) {
        LogPrint(BCLog::GOBJECT, "Object submission rejected because of rate check failure - hash = %s\n", strHash);
        throw JSONRPCError(RPC_INVALID_PARAMETER, "Object creation rate limit exceeded");
    }

    LogPrint(BCLog::GOBJECT, "Adding locally created governance object - %s\n", strHash);
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    { "bitcorn",           "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "bitcorn",           "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "bitcorn",           "gobject",                &gobject,                {} },
    { "bitcorn",           "voteraw",                &voteraw,                {} },
