                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        if (pool.existsProviderTxConflict(tx))
            return state.Invalid(ValidationInvalidReason::TX_MEMPOOL_POLICY, false, REJECT_DUPLICATE, "protx-dup");
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    CAmount ret = blockValue * 0.421; // 39.995% of block reward
    return ret;
}
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            undo.fCoinStake = alternate.fCoinStake; // proof-of-stake
        } else {
...
{
    bool fClean = true;
    bool fHasBestBlock = pspecialdb->VerifyBestBlock(pindex->GetBlockHash());
    if (!fHasBestBlock) {
        AbortNode("found specialDB inconsistency, you must reindex to continue");
    ...
    pspecialdb->WriteBestBlock(pindex->pprev->GetBlockHash());
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (pindex->pprev) {
        bool fHasBestBlock = pspecialdb->VerifyBestBlock(pindex->pprev->GetBlockHash());

        if (pindex->nHeight > 1 && !fHasBestBlock) {
            return AbortNode(state, "Found specialDB inconsistency, you must reindex to continue");
        }
    }
    ...
    if (pindex->nHeight <= Params().GetConsensus().nLastPoWBlock && block.IsProofOfStake())
    ...
    pspecialdb->WriteBestBlock(pindex->GetBlockHash());
    ...
    int nManualPruneHeight)
{
    ...
    try {
    {
        ...
        if (fDoFullFlush && !pcoinsTip->GetBestBlock().IsNull()) {
            ...
            if (!pspecialdb->CommitRootTransaction())
                return AbortNode(state, "Failed to commit specialDB");
            nLastFlush = nNow;
...
bool CChainState::DisconnectTip(CValidationState& state, const CChainParams& chainparams, DisconnectedBlockTransactions *disconnectpool)
{
    ...
    {
        auto dbTx = pspecialdb->BeginTransaction();
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSignature)
{
    ...
    // proof-of-stake: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < block.vtx.size(); i++)
        if (block.vtx[i]->IsCoinStake())
            return state.Invalid(ValidationInvalidReason::CONSENSUS, false, REJECT_INVALID, "bad-cs-missing", "coinstake in wrong position");

    // proof-of-stake: first coinbase output should be empty if proof-of-stake block
    ...
    // proof-of-stake: check coinbase timestamp
    ...
    if (block.IsProofOfStake() && fCheckMerkleRoot && fCheckSignature) {
...
static bool CheckIndexAgainstCheckpoint(const CBlockIndex* pindexPrev, CValidationState& state, const CChainParams& chainparams, const uint256& hash)
{
    ...
    if (pcheckpoint && nHeight < pcheckpoint->nHeight)
        return state.Invalid(ValidationInvalidReason::CONSENSUS, error("%s: forked chain older than last checkpoint (height %d)", __func__, nHeight));
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // Dont enforce the height in the input script anymore.
...
bool BlockManager::AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fProofOfStake)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
    uint256 hash = block.GetHash();
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (pindexPrev->nStatus & BLOCK_CONFLICT_CHAINLOCK)
            ...
            return state.Invalid(ValidationInvalidReason::BLOCK_INVALID_PREV, error("%s: prev block %s conflicts with chainlock", __func__, block.hashPrevBlock.ToString()), REJECT_INVALID, "bad-prevblk-chainlock");
...
bool ProcessNewBlockHeaders(const std::vector<CBlockHeader>& headers, CValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex, CBlockHeader *first_invalid)
{
    if (first_invalid != nullptr) first_invalid->SetNull();
    {
        ...
        for (const CBlockHeader& header : headers) {
            ...
            bool accepted = g_blockman.AcceptBlockHeader(header, state, chainparams, &pindex, header.nFlags & CBlockIndex::BLOCK_PROOF_OF_STAKE);
...
/** Store block on disk. If dbp is non-nullptr, the file is known to already reside on disk */
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock, bool* fSpamNode, bool fCheckPoS)
{
    const CBlock& block = *pblock;

    if (fSpamNode) *fSpamNode = false;
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock, bool *fSpamNode)
{
    AssertLockNotHeld(cs_main);

    {
        ...
        if (ret) {
            // Store to disk
            ret = ::ChainstateActive().AcceptBlock(pblock, state, chainparams, &pindex, fForceProcessing, nullptr, fNewBlock, fSpamNode);
        }
        if (!ret) {
    ...
    std::set<CBlockIndex*, CBlockIndexWorkComparator>& block_index_candidates)
{
    ...
    for (const std::pair<int, CBlockIndex*>& item : vSortedByHeight)
    {
        ...
        // proof-of-stake: calculate stake modifier checksum
        pindex->nStakeModifierChecksum = GetStakeModifierChecksum(pindex);
        if (ChainActive().Contains(pindex))
            if (!CheckStakeModifierCheckpoints(pindex->nHeight, pindex->nStakeModifierChecksum))
                return error("%s: failed stake modifier checkpoint height=%d, modifier=0x%016llx", __func__, pindex->nHeight, pindex->nStakeModifier);
    }
            ...
            try {
                ...
                uint256 hash = block.GetHash();
                {
                    ...
                    if (!pindex || (pindex->nStatus & BLOCK_HAVE_DATA) == 0) {
                      CValidationState state;
                      if (::ChainstateActive().AcceptBlock(pblock, state, chainparams, nullptr, true, dbp, nullptr, nullptr, false)) {
                          nLoaded++;
                      }
                      if (state.IsError()) {
                          break;
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        if (ReadBlockFromDisk(*pblockrecursive, it->second, chainparams.GetConsensus()))
                        {
                            ...
                            if (::ChainstateActive().AcceptBlock(pblockrecursive, dummy, chainparams, nullptr, true, &it->second, nullptr, nullptr, false))
                            {
                                nLoaded++;
